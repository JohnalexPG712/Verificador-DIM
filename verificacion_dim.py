...
# =============================================================================
# CLASE 2: COMPARACIÓN DE DATOS (CORREGIDA PARA BULTOS Y ERRORES PANDAS)
# =============================================================================
...
class ComparadorDatos:
        ...
        for emparejamiento in emparejamientos:
            ...
            for campo, (campo_dian, _) in self.campos_subpartida_arancelaria.items():
                ...
                if mostrar_emojis:
                    ...
                    fila_reporte[f"{campo_dian} DI"] = f"{emoji_di} {self.formatear_numero_entero(valor_dian, campo_dian)}"
                    fila_reporte[f"{campo_dian} Subpartida"] = f"{emoji_sub} {self.formatear_numero_entero(valor_subpartida, f'{campo_dian} Subpartida')}"
                else:
                    fila_reporte[f"{campo_dian} DI"] = self.formatear_numero_entero(valor_dian, campo_dian)
                    fila_reporte[f"{campo_dian} Subpartida"] = self.formatear_numero_entero(valor_subpartida, f"{campo_dian} Subpartida")

            # Bultos en filas individuales: Solo mostrar valor
            for campo, (campo_dian, _) in self.campos_bultos.items():
                ...
                fila_reporte[f"{campo_dian} DI"] = self.formatear_numero_entero(valor_dian, campo_dian) if self.es_valor_valido(valor_dian) else None
                fila_reporte[f"{campo_dian} Subpartida"] = self.formatear_numero_entero(valor_subpartida, campo_dian) if self.es_valor_valido(valor_subpartida) else None
    ...
    def _agregar_totales_multiples_subpartidas(self, reporte_filas, datos_dian, datos_subpartidas):
        """Agrega fila de totales CORREGIDA para bultos"""
    ...
    def _agregar_totales_una_subpartida(self, reporte_filas, datos_dian, fila_subpartida):
        """Agrega fila de totales CORREGIDA para bultos (caso una subpartida)"""
        ...
        # --- LÓGICA CORREGIDA PARA BULTOS (UNA SUBPARTIDA) ---
        valor_bultos_di_consolidado = 0
        if '74. Número de Bultos' in datos_dian.columns:
            s_bultos = pd.to_numeric(datos_dian['74. Número de Bultos'], errors='coerce')
            unicos = s_bultos.dropna().unique()
        ...
        if fila_subpartida is not None and not isinstance(fila_subpartida, dict) and 'numero_bultos' in fila_subpartida:
             valor_bultos_excel = fila_subpartida['numero_bultos']
        elif isinstance(fila_subpartida, dict):
    ...
    def generar_reporte_comparacion(self, datos_dian, datos_subpartidas, output_path):
        ...
        if not df_reporte.empty:
            try:
                # Asegurar conversión numérica para columnas de bultos (excepto la fila de totales que tiene texto/emojis)
                for col in df_reporte.columns:
                    if '74. Número de Bultos' in col:
                        # Solo convertir las filas que no son de totales para mantener el formato numérico en Excel
                        mask_totales = df_reporte['4. Número DI'].str.contains('VALORES ACUMULADOS', na=False)
                        # CORRECCIÓN AQUÍ: Cambiado 'ignore' por 'coerce' para evitar FutureWarning
...
class ValidadorDeclaracionImportacionCompleto:
    def __init__(self):
        ...
        self.patrones = {
            ...
            "134. Levante No.": [r"134\s*\.?\s*Levante\s*No\.?\s*(\d+)"],
    ...
    def procesar_validacion_completa(self, carpeta_pdf, archivo_salida=None):
        ...
        if all_results:
            ...
            try:
                ...
                print(f"   • Total: {len(todas_decs)}\n   • Errores: {err_count}\n   • Correctas: {len(todas_decs)-err_count}")
...
def main():
    CARPETA_BASE = r"E:\Users\Lenovo\Desktop\PYTHON\DI\Junior Deposito 401\SLIND 401\Junior 850341"
    ...
    try:
        ...
        if datos_dian is not None and not datos_dian.empty and not datos_sub.empty:
            ComparadorDatos().generar_reporte_comparacion(datos_dian, datos_sub, EXCEL_OUTPUT_COMPARACION)
        else: print("❌ Datos insuficientes para comparación")

        print("\n📋 2. VALIDACIÓN ANEXOS FMM vs DIM")
        ValidadorDeclaracionImportacionCompleto().procesar_validacion_completa(CARPETA_BASE, EXCEL_OUTPUT_ANEXOS)
